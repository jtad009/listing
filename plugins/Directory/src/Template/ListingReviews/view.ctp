<?php
/**
 * @var \App\View\AppView $this
 * @var \Cake\Datasource\EntityInterface $listingReview
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Listing Review'), ['action' => 'edit', $listingReview->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Listing Review'), ['action' => 'delete', $listingReview->id], ['confirm' => __('Are you sure you want to delete # {0}?', $listingReview->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Listing Reviews'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Listing Review'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Listings'), ['controller' => 'Listings', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Listing'), ['controller' => 'Listings', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="listingReviews view large-9 medium-8 columns content">
    <h3><?= h($listingReview->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= h($listingReview->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Listing') ?></th>
            <td><?= $listingReview->has('listing') ? $this->Html->link($listingReview->listing->id, ['controller' => 'Listings', 'action' => 'view', $listingReview->listing->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Ip') ?></th>
            <td><?= h($listingReview->ip) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($listingReview->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($listingReview->modified) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Rating') ?></h4>
        <?= $this->Text->autoParagraph(h($listingReview->rating)); ?>
    </div>
</div>
